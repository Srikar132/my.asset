...
- `data/experience.ts` — `ExperienceItem[]` (company, role, type, period, description). Default export.
...
| `FreelanceWork.tsx` | Freelance section (`data/freelance.ts`). **Sticky stacked "pages"**: each project is a `sticky top-0 h-screen` panel with rounded top + shadow and increasing `z-index`, so the next screen slides up and sits on top of the previous (native sticky → smooth with Lenis, no pin glitch). Per-panel GSAP reveal on enter (`start top 60%`, once): card rises/scales in, info (`.fw-rise`) staggers up. Card left, info right (display-font name + `[OPEN]`, mono category/role, summary, VISIT SITE). Accent gradient card = placeholder; real screenshots at `p.image` (`/creonex.png`, `/nala-armoire.png`, `/treekart.png`) overlay it, `onError`-hidden if missing. |
...
| `Experience.tsx` | `id="experience"`. Work history from `data/experience.ts`. AboutMe-style scroll reveal (line grows → mono `EXPERIENCE` label → rows stagger). Each row: left `period`/`type` (mono), right `role` + `company` + `description`; hover highlight. |
...
| `Footer.tsx` | Reference-matched: big tagline → braced (DrawSVG) "LET'S WORK TOGETHER ↗" mailto CTA → animated divider → mono meta row (email, INSTAGRAM, LINKEDIN / live **IST clock** `Asia/Kolkata`, BACK TO TOP via `useLenis().scrollTo(0)`, SHOW GRID toggle overlay / DESIGNED BY) → giant **SRIKAR DEV** name (Unica One, clip-reveal up). Scroll-triggered reveal timeline (`once`). |
